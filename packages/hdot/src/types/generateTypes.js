...
const { getSpec, getSpecByTagName, getAttrSpecs } = require("@markuplint/ml-spec");
...
htmlSpec.specs.filter(spec => !spec.name.includes(':')).forEach(spec => {
  typeMap.push(`  /** ${spec.deprecated ? '@deprecated ': ''}${spec.description} */`)
  typeMap.push(`  "${spec.name}": ${spec.name}Element;`);
})
typeMap.push(`}`);
content.push(typeMap.join('\n'));

htmlSpec.specs.filter(s => !s.name.includes(':')).forEach(s => {
  const spec = { name: s.name, description: s.description, ...getSpecByTagName(s.name, htmlSpec)};
  
  const typeDef = [
    `type ${spec.name}Element = {`
  ];
  ...
  const getAttrType = (attr) => {
    ...
    if (type.enum && type.enum.length > 0) {
      let stringEnum = (type.enum).reduce((prev, curr) => prev += `"${curr}" | `, '');
    ...
    return `DomTypeMap[${strType.startsWith('"') ? strType : `\`${strType}\``}]`;
  }

  const getAttr = (attr) => {
    attr.description && typeDef.push(`  /** ${attr.deprecated ? '@deprecated ': ''}${attr.description} */`);
    typeDef.push(`  "${attr.name}": (val: ${getAttrType(attr)}) => HTMLElement<"${spec.name}">;`);
  }

  getAttrSpecs(spec.name, htmlSpec).filter(attr => attr.type !== 'FunctionBody').forEach(attr => {
    getAttr(attr);
  })
